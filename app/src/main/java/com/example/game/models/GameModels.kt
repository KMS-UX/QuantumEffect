...
enum class Outfit(val displayName: String, val description: String, val bonusDesc: String, val hpMult: Float = 1.0f, val defMult: Float = 1.0f, val spdMult: Float = 1.0f) {
...
enum class WeaponType {
    MELEE, PISTOL, RIFLE, HEAVY, ENERGY, QUANTUM
}
...
enum class Weapon(
    ...
    REALITY_SHREDDER("Reality Shredder", WeaponType.QUANTUM, 200, 250, 20, "Warp technology that temporarily creates localized rift breaches.", "+30% Void corruption damage")
}

enum class ModChip(val displayName: String, val bonusDesc: String, val category: String) {
...
enum class Faction(
    ...
    AURELIAN_ORDER("Aurelian Order", "A technomystic theocracy dedicated to the preservation of ancient space coordinates.", "Lawful • Structured", "Aurelian Prime", "Advanced Tech Access", "Discount on Tech & Medical Supplies", QuantumNeonBlue),
    EMBERPACT("Emberpact", "A fierce coalition of frontier mining clans and survivalist raiders.", "Chaotic • Fierce", "Kharag Wastes", "Heavy Weapon Mastery", "Increased loot from defeated mechanoids", QuantumNeonOrange),
    VOID_SEEKERS("Void Seekers", "Mystics and outcasts utilizing dark-matter technology to transcend physical boundaries.", "Mysterious • Insightful", "Nyx Expanse", "Void Magic Mastery", "Increases Void anomaly resistances by 30%", QuantumNeonPurple),
    IRONWARD("Ironward", "A highly militarized industrial union focused on defense, security, and robotic engineering.", "Lawful • Practical", "Vestra Industrial", "Armored Vehicle Support", "Ability to deploy battle turrets & drones", QuantumNeonGreen),
    SILENT_VEIL("Silent Veil", "A secretive network of shadow spies, data brokers, and cutthroat herbalists.", "Neutral • Stealthy", "Unknown", "Stealth Expertise", "Black market trades & hidden path access", QuantumNeonRed)
}
...
data class CompanionRecord(
    ...
    val portraitSymbol: String, // simple text icon/avatar
    val level: Int = 1,
...
enum class ActiveSkill(val displayName: String, val mpCost: Int, val description: String, val color: Color) {
    QUANTUM_BURST("Quantum Burst", 20, "Deal 120 Magic damage to all active threats.", QuantumNeonPurple),
    TIME_STEP("Time Step", 15, "Gain +50% evasion and blink forward, dealing 40 Kinetic damage.", QuantumNeonBlue),
    PHASE_SHIELD("Phase Shield", 25, "Create an energy barrier absorbing up to 150 incoming damage.", QuantumNeonGreen),
    RESONANCE_WAVE("Resonance Wave", 18, "Emit an anomaly wave reducing enemy armor by 40% for 3 turns.", QuantumNeonOrange),
    GRAVITY_WELL("Gravity Well", 30, "Pull enemies together, trapping them and dealing 30 damage per turn.", QuantumNeonPurple),
    HEALING_PULSE("Healing Pulse", 22, "Activate dynamic bio-regenerators to restore 150 HP immediately.", QuantumNeonGreen),
    VOID_TOUCH("Void Touch", 12, "Empower strikes to convert 30% of physical damage into health.", QuantumNeonPurple),
    TEMPORAL_LOOP("Temporal Loop", 35, "Rewind timeline by 3 seconds, removing all debuffs and restoring 80 HP.", QuantumNeonBlue),
    ENERGY_OVERLOAD("Energy Overload", 28, "Enter an overcharged state, boosting ATK and MAG by 50% for 2 turns.", QuantumNeonOrange)
}
...
data class PointOfInterest(
    ...
    val biome: Biome,
    val description: String,
    val dangerLevel: String, // Minor, Moderate, Severe, Critical, Apocalyptic
    val resource: String, // Voidium, Neutronite, Singularium, etc.
...
data class Enemy(
    ...
    val speed: Int,
    val description: String,
    val resistanceType: String, // Void, Energy, Kinetic, Expl
